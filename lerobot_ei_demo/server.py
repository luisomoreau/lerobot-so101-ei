...
from lerobot_ei_demo.camera import camera_index, mjpeg_stream
from lerobot_ei_demo.camera_registry import CameraRegistry
from lerobot_ei_demo.hardware import LeRobotController
from lerobot_ei_demo.session import RobotSession, discover_ports
...
def configure_cameras(
    ...
    try:
        return camera_registry.configure([camera.model_dump() for camera in cameras])
...
def camera_stream(camera_id: str) -> StreamingResponse:
    ...
    return StreamingResponse(
        mjpeg_stream(camera_id), media_type="multipart/x-mixed-replace; boundary=frame"
    )
...
def inference_status() -> dict[str, str | float | None]:
    return inference.status()


@app.post("/api/inference/start")
def start_inference(selection: InferenceSelection) -> dict[str, str | float | None]:
    if not 0 <= selection.confidence <= 1:
        raise HTTPException(
            status_code=422, detail="confidence must be between 0 and 1"
        )
    try:
        return inference.start(
            selection.model_id, selection.camera_id, selection.confidence
        )
...
def stop_inference() -> dict[str, str | float | None]:
